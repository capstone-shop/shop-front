...
function ProductEdit() {
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      ...
      try {
        const response = await getProductDetail({ id: Number(id) });
        const product = response.merchandise;
        ...
        setSelectedCondition(product.merchandiseState || '');
        setDirectDeal(
          product.transactionMethod === 'DIRECT' ||
            product.transactionMethod === 'BOTH'
        );
        setDeliveryDeal(
        ...
        // 카테고리 초기화
        const categories = product.category; // 대, 중, 소 카테고리 배열
        if (categories.length > 0) {
          setSelectedCategory(categories[0].id); // 대 카테고리 설정
          await fetchSubCategories(categories[0].id); // 중 카테고리 로드

          if (categories.length > 1) {
            setSelectedSubCategory(categories[1].id); // 중 카테고리 설정
            await fetchSmallCategories(categories[1].id); // 소 카테고리 로드

            if (categories.length > 2) {
              setSelectedSmallCategory(categories[2].id); // 소 카테고리 설정
            }
          }
        }
      } catch (err) {
        console.error('상품 데이터 로드 중 오류 발생:', err);
      }
    };
  ...
  const handleProductEdit = async (e: React.FormEvent) => {
    ...
    const productData = {
      id: Number(id), // URL에서 가져온 id를 포함
      images,
      name: formData.name,
      categoryId:
        selectedSmallCategory || selectedSubCategory || selectedCategory,
    ...
    try {
      await putProduct(productData); // 반환값 없음
      setModalMessage('상품이 성공적으로 수정되었습니다!');
      setIsModalOpen(true);
      // navigate('/');
    } catch (error) {
      console.error('상품 등록 중 오류:', error);
      setModalMessage('상품 수정에 실패했습니다. 다시 시도해주세요.');
      setIsModalOpen(true);
    }
  };
  ...
  const closeModal = () => {
    ...
    if (error) {
      navigate(`/productEdit/${id}`); // 실패시 수정페이지
    } else {
      navigate(`/productDetail/${id}`); // 성공 시 메인 페이지로 이동
